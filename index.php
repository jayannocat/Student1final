...
<div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab" tabindex="0">
        ...
        <tbody align="center">
            ...
            <?php
            $id = $_SESSION['u_id'];
            $getData = $conn->prepare("SELECT COUNT(*) FROM students_record WHERE user_id=?");
            $getData->execute([$id]);
            ...
            $getItems = $conn->prepare("SELECT * FROM students_record WHERE user_id=? LIMIT $offset, $itemsPerPage");
            $getItems->execute([$id]);
            foreach ($getItems as $data) { ?>
                ...
                <tr>
                    ...
                    <td class="px-md-4"><?= $data['firstName'] ?></td>
                    <td class="px-md-4"><?= $data['lastName'] ?></td>
                    <td class="px-md-4"><?= $data['course'] ?></td>
                   

                    <td class="px-md-1">
                    <a class="text-decoration-none " href="add.php?update&id=<?= $selects['user_id'] ?>" class="text-decoration-none">✏</a>
                                    |
                                    <a class="text-decoration-none" href="process.php?delete&id=<?= $selects['user_id'] ?>" class="text-decoration-none">❌</a>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
...
<?php
    if (isset($_GET['update'])) { ?>
        ...
        $getUser = $conn->prepare("SELECT * FROM students_record WHERE user_id = ?");
        $getUser->execute([$id]);
